...
// --- NEW ADDITION FOR SPEED CONTROL ---
// Remove jumpers from L298N and connect ENA/ENB pins
const int ENA = 5;  // PWM Pin for Motor A speed
const int ENB = 6;  // PWM Pin for Motor B speed

// Set Robot Speed (0-255)
// Try 120 for slow/safe speed, 150 for medium, 255 for max.
...
// ==========================================
// 4. CLASS: MOTOR ABSTRACTION LAYER (UPDATED)
// ==========================================
...
class DriveTrain {
  private:
    int p1, p2, p3, p4;
    int enA, enB; // Speed control pins
    int speed;    // Current speed value

  public:
    // Updated Constructor to accept ENA/ENB pins
    ...
    void begin() {
      ...
      pinMode(enA, OUTPUT); pinMode(enB, OUTPUT); // Configure PWM pins
      set(HALT);
    }
...
// Pass Motor Pins AND Speed Control Pins (ENA, ENB)
...
void setup() {
  ...
  Serial.println("System Ready: FULL VERSION (Speed Control Active)");
}
